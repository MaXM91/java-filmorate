...
@Component @Qualifier("FilmDbStorage") public class FilmDbStorage implements FilmStorage {
        ...
        try {
            return jdbcTemplate.queryForObject("SELECT * FROM films WHERE id = ?",
                (rs, rowNum) -> new Film(rs.getInt("id"), rs.getString("name"), rs.getString("description"),
                    readFilmGenre(rs.getInt("id")), readFilmMpa(rs.getInt("id")), readUsersLike(id),
    ...
    public List<Film> get() {
        ...
        return jdbcTemplate.query("SELECT * FROM films",
            (rs, rowNum) -> new Film(rs.getInt("id"), rs.getString("name"), rs.getString("description"),
                readFilmGenre(rs.getInt("id")), readFilmMpa(rs.getInt("id")), readUsersLike(rs.getInt("id")),
                rs.getDate("releaseDate")
        ...
        try {
            ...
            return jdbcTemplate.query(
                "SELECT f.id, f.name, f.description, f.releasedate, f.duration, COUNT(l.user_id) AS rate\n" +
                    ...
                    "ORDER BY rate DESC\n" + "LIMIT ? ",
                (rs, rowNum) -> new Film(rs.getInt("id"), rs.getString("name"), rs.getString("description"),
                    readFilmGenre(rs.getInt("id")), readFilmMpa(rs.getInt("id")), rs.getInt("rate"),
                    rs.getDate("releaseDate")
    ...
    private List<Genre> changeFilmGenre(Film film) {
        ...
        for (Genre genre : film.getGenres()) {
            ...
            jdbcTemplate.update("INSERT INTO film_genre (film_id, genre_id) VALUES(?,?)", film.getId(),
                genre.getId()); // Ввести пару

            Genre fullGenre = jdbcTemplate.queryForObject("SELECT * FROM genre WHERE genre_id = ?",
                (rs, rowNum) -> new Genre(rs.getInt("genre_id"), rs.getString("name")), genre.getId());
    ...
    private Mpa changeFilmMpa(Film film) {
        jdbcTemplate.update("INSERT INTO film_mpa (film_id, mpa_id) VALUES(?, ?)", film.getId(), film.getMpa()
            .getId());
