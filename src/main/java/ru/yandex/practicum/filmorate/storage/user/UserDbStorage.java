...
public class UserDbStorage implements UserStorage {
        ...
        try {
            ...
            for (int i = 0; i < rs.getRow(); i++) {
                if (users.containsKey(rs.getInt("id"))) {
                    ...
                    friends.add(rs.getInt("user_id_from"));
                    users.get(rs.getInt("id")).setUserFriends(friends);

                    rs.next();
                } else {
                    User user = new User(rs.getInt("id"),
                        new HashSet<>(rs.getInt("user_id_from")),
    ...
    public List<User> getFriends(Integer id) {
        try {
            ...
            for (int i = 0; i < rs.getRow(); i++) {
                if (users.containsKey(rs.getInt("user_id_to"))) {
                    friends = users.get(rs.getInt("user_id_to")).getUserFriends();
                    friends.add(rs.getInt("user_id_to"));
                    users.get(rs.getInt("user_id_to")).setUserFriends(friends);

                    rs.next();
                } else {
                    User user = new User(rs.getInt("user_id_to"),
        ...
        try {
            ...
            for (int i = 0; i < rs.getRow(); i++) {
                if (users.containsKey(rs.getInt("user_id_to"))) {
                    ...
                    User user = new User(rs.getInt("user_id_to"), new HashSet<>(rs.getInt("user_id_to")),
