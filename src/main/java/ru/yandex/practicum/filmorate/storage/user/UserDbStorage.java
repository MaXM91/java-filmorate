...
public class UserDbStorage implements UserStorage {
    ...
    public User found(Integer id) {
        try {
            User user = jdbcTemplate.queryForObject("SELECT * FROM users WHERE id = ?",
                (rs, rowNum) -> new User(rs.getInt("id"), rs.getString("email"), rs.getString("login"),
    ...
    public List<User> get() {
        ...
        return jdbcTemplate.query("SELECT * FROM users", (rs, rowNum) -> {
            User user = new User(rs.getInt("id"), rs.getString("email"), rs.getString("login"), rs.getString("name"),
                rs.getDate("birthday")
                    .toLocalDate());
    ...
    public void addFriend(Integer id, Integer idFriend) {
        ...
        if (checkFriends(id, idFriend)) {
            jdbcTemplate.update("UPDATE friends SET approved_by_to = 1 WHERE user_id_from = ? AND user_id_to = ?", id,
                idFriend);
            jdbcTemplate.update("UPDATE friends SET approved_by_to = 1 WHERE user_id_from = ? AND user_id_to = ?",
                idFriend, id);
        }
    }
    ...
    public void deleteFriend(Integer id, Integer idFriend) {
        ...
        if (checkFriends(id, idFriend)) {
            jdbcTemplate.update("UPDATE friends SET approved_by_to = 0 WHERE user_id_from = ? AND user_id_to = ?", id,
                idFriend);
            jdbcTemplate.update("UPDATE friends SET approved_by_to = 0 WHERE user_id_from = ? AND user_id_to = ?",
                idFriend, id);
        }
    }
    ...
    private List<Integer> foundFriendsIds(Integer id) {
        return jdbcTemplate.query("SELECT user_id_to FROM friends WHERE user_id_from = ?",
            (rs, rowNum) -> rs.getInt("user_id_to"), id);
    }
